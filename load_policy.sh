...
cat /mnt/ansible-ec2/policy/new_instance.yml \
	...
	| conjur policy load --as-role layer:ansible

# Populate policy variables
conjur variable value ansible/ec2/bootstrap_private_key | conjur variable values add "$POLICY_ID"/private_key
...
conjur role revoke_from policy:$POLICY_ID layer:ansible
